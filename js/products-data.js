...
function render(products=[]){
 ...
 tableBody.innerHTML=products.length?products.slice().reverse().map(p=>`<tr><td><strong>${safe(p.name)}</strong>${p.images?.[0]?`<br><img src="${safe(p.images[0])}" alt="" style="width:48px;height:48px;object-fit:cover;border-radius:8px;margin-top:7px">`:''}</td><td>${safe(p.sku||'-')}</td><td>${money.format(Number(p.price||0))}</td><td>${Number(p.stock||0)}</td><td>${(p.channels||[]).map(c=>'<span class="pill" title="'+safe(p.publishing?.[c]?.status||'waiting_verification')+'">'+publishLabel(p,c)+' '+safe(c)+'</span>').join(' ')||'—'}</td><td><span class="status-pill ${p.verificationStatus==='pending'?'status-pending':''}">${safe(p.verificationStatus||'Pending')}</span></td><td><button class="button" type="button" data-view-product="${safe(p.id)}">View</button></td></tr>`).join(''):'<tr><td colspan="7">No products yet. Add your first product above.</td></tr>';
}
function openProduct(id){const p=loadedProducts.find(x=>x.id===id);if(!p)return;const detail=document.querySelector('[data-product-detail]');detail.innerHTML=`${p.images?.[0]?`<img src="${safe(p.images[0])}" alt="${safe(p.name)}">`:''}<span class="status-pill status-pending" style="display:inline-block;margin-top:15px">${safe(p.verificationStatus||'Pending')}</span><h2>${safe(p.name)}</h2><div class="preview-price">${money.format(Number(p.price||0))}</div><p>${safe(p.desc||'')}</p><p><b>SKU:</b> ${safe(p.sku||'-')} · <b>Stock:</b> ${Number(p.stock||0)}</p><p><b>Selling area:</b> ${safe(p.targetArea||'Not set')}</p><p><b>Target population:</b> ${Number(p.targetPopulation||0).toLocaleString('en-ZA')}</p><p><b>Reach fee:</b> ${Number(p.reachFee||0)===0?'Free':'R'+Number(p.reachFee)}</p><p><b>Publishing:</b></p><div class="selected-channels">${(p.channels||[]).map(c=>'<span class="pill">'+publishLabel(p,c)+' '+safe(c)+' · '+safe((p.publishing?.[c]?.status||'waiting_verification').replaceAll('_',' '))+'</span>').join(' ')||'—'}</div>`;document.querySelector('[data-product-modal]').classList.add('show')}
