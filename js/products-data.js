...
function updateTargetSummary(){const area=form.elements.targetArea?.value?.trim()||'';const radius=form.elements.targetRadius?.value||'50';const gender=form.elements.targetGender?.selectedOptions?.[0]?.textContent||'All genders';const node=document.querySelector('[data-target-summary]');if(node)node.innerHTML=area?`<strong>Selling area:</strong> ${safe(area)} · <strong>Radius:</strong> ${radius==='national'?'All South Africa':safe(radius)+' km'} · <strong>Audience:</strong> ${safe(gender)}`:'Enter an area to see your selling target.'}
form?.elements.targetArea?.addEventListener('input',updateTargetSummary);form?.elements.targetRadius?.addEventListener('change',updateTargetSummary);form?.elements.targetGender?.addEventListener('change',updateTargetSummary);
